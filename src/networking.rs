...
impl NetworkManager{
    ...
    fn process_commands(&mut self, orders: &mut VecDeque<Vec<Order>>, buf: &[u8], turn_t: u64, conf: &SystemConfig){
        let rec_turn_t: u64 =  deserialize_from(&buf[0..8]).unwrap();
        let rec_turn = rec_turn_t as usize;
        let turn = turn_t as usize;
        let mut rec_orders: VecDeque<Vec<Order>> = deserialize_from(&buf[8..]).unwrap();
    ...
    pub fn send_commands(&mut self, orders: &VecDeque<Vec<Order>>, turn: u64){
        let mut buf = Vec::new();
